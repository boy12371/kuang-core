import * as core from "@nestjs/core";
import * as common from "@nestjs/common";
import * as cqrs from "@nestjs/cqrs";
import * as elasticsearch from "@nestjs/elasticsearch";
import * as graphql from "@nestjs/graphql";
import * as jwt from "@nestjs/jwt";
import * as microservices from "@nestjs/microservices";
import * as mongoose from "@nestjs/mongoose";
import * as passport from "@nestjs/passport";
import * as swagger from "@nestjs/swagger";
import * as testing from "@nestjs/testing";
import * as typeorm from "@nestjs/typeorm";
import * as websockets from "@nestjs/websockets";
import * as version from "@nestjsx/api-version";
import * as crud from "@nestjsx/crud";
export { core, common, cqrs, elasticsearch, graphql, jwt, microservices, mongoose, passport, swagger, testing, typeorm, websockets, version, crud };
